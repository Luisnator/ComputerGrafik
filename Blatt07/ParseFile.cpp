	...
	if (inputstream.is_open())
	{
		...
		while (getline(inputstream, line))
		{
			...
			else if (type == "f")
			{
				...
				stringstream s(values2[0]);
				cout << "test";
				string f1;
				getline(s,f1, '/');
				float first = strtof(f1.c_str(), 0);
				
				string f2;
				getline(s, f2, '/');
				float firstt = strtof(f2.c_str(), 0);

				string f3;
				getline(s, f3, '/');
				float firstn = strtof(f3.c_str(),0);
				...
				for (int i = 0; i < values2.size()-2; i++)
				{
					...
					vector<float> texi;
					vector<float> normi;
					stringstream s1(values2[i+1]);
					stringstream s2(values2[i + 2]);
					//Indi first
					indi.push_back(first);
					//Indi second
					string i1;
					getline(s1, i1, '/');
					indi.push_back(strtof(i1.c_str(), 0));
					//Indi third
					string i2;
					getline(s2, i2, '/');
					indi.push_back(strtof(i2.c_str(), 0));
					...
					string n1;
					getline(s1, n1, '/');
					normi.push_back(strtof(n1.c_str(), 0));
					//normi third
					string n2;
					getline(s2, n2, '/');
					normi.push_back(strtof(n2.c_str(), 0));
					indinorm.push_back(normi);
			...
			{
				texturcoord.push_back(glm::vec2(strtof(values2[0].c_str(), 0), strtof(values2[1].c_str(), 0)));
			}
			else if (type == "vn")
			{
				normals.push_back(glm::vec3(strtof(values2[0].c_str(), 0), strtof(values2[1].c_str(), 0), strtof(values2[2].c_str(),0)));
			}
		}
